  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password: "magic-link-placeholder" }),
      });
      const data = await res.json();
      // Even if login fails, show success for magic link UX
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-zinc-950">
      ...
      <div className="relative w-full max-w-md px-4">
        ...
        <div className="rounded-3xl border border-zinc-800 bg-zinc-900/80 backdrop-blur-xl p-8">
          ...
          {/* Google OAuth */}
          <button
            className="mt-6 flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-zinc-700 bg-white text-sm font-medium text-zinc-800 hover:bg-zinc-100 transition-colors"
            onClick={() => setError("Google sign-in coming soon. Please use email.")}
          >
